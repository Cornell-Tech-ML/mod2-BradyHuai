...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return out * (tensor(1.0) - out) * grad_output
...
class Sum(Function):
    ...
    def backward(
        ...
        if dim is None:
            return t1.expand(grad_output)  # TODO
        else:
            return t1.expand(grad_output), zeros(dim.shape)
...
class Permute(Function):
    ...
    def forward(ctx: Context, t1: Tensor, order: Tensor) -> Tensor:
        ...
        #     range(len(t1.shape))
        # ), f"Must give a position to each dimension. Shape: {t1.shape} Order: {order}"

        orders = order.to_numpy().astype(int).tolist()
        ctx.save_for_backward(orders)

        return t1._new(t1._tensor.permute(*orders))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        reverse_order = [0] * len(orders)
        for i in range(len(orders)):
            reverse_order[int(orders[i])] = i
